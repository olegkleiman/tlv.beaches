...
struct BindingWrapper: Identifiable {
    let id = UUID()
    let label: String
    var value: String
}
...
struct TokenView: View {
    ...
    func GetShortDateFormatter() -> DateFormatter {
    ...
    var body: some View {
        VStack {
            ...
            let dateFormatter = GetShortDateFormatter()
            let _exp = try? dateFormatter.string(from:self.expiresAt)
            
            let inputFields = [BindingWrapper(label: "Name", value: self.name),
                               BindingWrapper(label: "Email", value: self.email),
                               BindingWrapper(label: "Issuer", value: self.issuer),
            ...
            .onAppear {
                if let id_token = jsonTokens?.id_token {
                    ...
                    self.expiresAt = jwt?.expiresAt ?? Date()
                ...
                HStack {
                    ...
                    Button {
                        Task {
                            ...
                            let url = "https://TlvfpB2CPPR.b2clogin.com/TlvfpB2CPPR.onmicrosoft.com/B2C_1A_B2C_1_ROPC_KIEV_RP/oauth2/v2.0/token"
                            
                            let parameters: [String: String?] = [
                                "grant_type": "refresh_token",
                                "client_id": "8739c7f1-e812-4461-b9c8-d670307dd22b",
                                "scope": "openid offline_access https://TlvfpB2CPPR.onmicrosoft.com/8739c7f1-e812-4461-b9c8-d670307dd22b/TLV.Digitel.All",
...
//                            try await Task.sleep(nanoseconds: UInt64(seconds * Double(NSEC_PER_SEC)))
                            AF.request(url, method: .post, parameters: parameters,
                                       encoder: .urlEncodedForm, headers: headers)
    ...
    //                        .responseJSON { response in
            ...
            Button {
                ...
                KeychainWrapper.standard.removeObject(forKey: "sso_token")
...
let sampleTokens = DecodableTokens(
